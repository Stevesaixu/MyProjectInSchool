...
<script type="text/javascript">
	$(document).ready(function(){
	    ...
	    $("#addBtn").fancybox({
	        ...
	        'onClosed' : function() { 
	        	window.location.href = '<%=path%>/adm/adm_list.jsp';
	        }
	    });
	...
	/** 模糊查询来电用户  **/
	function search(){
		$("#submitForm").attr("action", "<%=path%>/selectHouse.do?page=" + 1).submit();
	}

	/** 新增   **/
	function add(){
		$("#submitForm").attr("action", "<%=path%>/adm/adm_add_house.jsp").submit();	
	}
...
//	 			alert(fyXqCode);
	...
	function del(fyID){
		...
		if(fyID == '') return;
		if(confirm("您确定要删除吗？")){
			$("#submitForm").attr("action", "<%=path%>/deleteHouse.do?id=" + fyID).submit();			
		}
	}
				...
				<div class="ui_tb">
					<table class="table" cellspacing="0" cellpadding="0" width="100%" align="center" border="0">
						...
						<c:forEach items="${sessionScope.house}" var="h">
							<tr>
								<td><input type="checkbox" name="IDCheck" value="${h.house_id}" class="acb" /></td>
								
								<td>${h.house_address}</td>
								<td>${h.house_storey}</td>
								<td>${h.house_large}</td>
								<td>${h.house_decorate}</td>
								<td>${h.house_type}</td>
								<td>${h.house_floor}</td>
								<td>${h.house_rentWay}</td>
								<td>${h.house_rentMoney}</td>
								<td>${h.house_deposit}</td>
								<td>${h.house_tag}</td>
								<td>${h.house_state}</td>
								<td>${h.house_user}</td>
								<td>${h.house_phone}</td>
								<td>
									<a href="selectHouseById.do?id=${h.house_id}" class="edit">编辑</a>
									<a href="javascript:del('${h.house_id}');">删除</a>
								</td>
							</tr>
					</c:forEach>
					</table>
				...
				<div class="ui_tb_h30">
					...
					<c:choose>
						<c:when test="${sessionScope.page.pageNow - 1 > 0}">
							 <a href="selectHouse.do?pageNow=1" class="ui_input_btn01">首页</a>
							<a href="selectHouse.do?pageNow=${sessionScope.page.pageNow - 1}" class="ui_input_btn01">上一页</a>
							<!-- <input type="button" value="上一页" class="ui_input_btn01" /> -->
						</c:when>
					</c:choose>
					<c:choose>
						<c:when test="${sessionScope.page.pageNow + 1 <= sessionScope.page.totalPageCount}">
							 <a href="selectHouse.do?pageNow=${sessionScope.page.pageNow + 1}" class="ui_input_btn01">下一页</a>
							<a href="selectHouse.do?pageNow=${sessionScope.page.totalPageCount}" class="ui_input_btn01">尾页</a>
							<!-- <input type="button" value="上一页" class="ui_input_btn01" /> -->
						</c:when>
